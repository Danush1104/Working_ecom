...
export default function Dashboard() {
 ...
 {/* Header */}
 <div>
 <h1 className="text-3xl font-space font-bold text-text-primary tracking-tight mb-2">
 ...
 <h3 className="text-lg font-semibold text-text-primary mb-1">Payment Health</h3>
 ...
 <Link to="/admin/orders" className="text-xs text-primary hover:text-text-primary flex items-center transition-colors">
 View All <ChevronRight className="h-3 w-3 ml-1" />
 </Link>
 </div>
 <div className="overflow-x-auto">
 <table className="w-full text-left text-sm text-text-secondary">
 <thead className="bg-bg-card border-b border-border-subtle text-xs uppercase text-text-secondary/80">
 <tr>
 ...
 <tr key={order.order_id} className="hover:bg-bg-secondary transition-colors">
 ...
 <td className="px-5 py-3 text-text-primary font-medium">{formatCurrency(order.total_amount)}</td>
 ...
 <div className="bg-bg-card/40 backdrop-blur-xl rounded-2xl border border-border-subtle overflow-hidden">
 <div className="p-5 border-b border-border-subtle flex items-center justify-between">
 <h3 className="text-base font-semibold text-text-primary">Inventory Health</h3>
 <Link to="/admin/inventory" className="text-xs text-primary hover:text-text-primary flex items-center transition-colors">
 ...
 return (
 <tr key={item.product_id} className="hover:bg-bg-secondary transition-colors">
 ...
 <td className="px-5 py-3 font-medium text-text-primary">{item.available || 0}</td>
 <td className="px-5 py-3 text-right">
 {(item.available || 0) === 0 ? (
 ...
 <div className="bg-bg-card/40 backdrop-blur-xl rounded-2xl border border-border-subtle overflow-hidden">
 <div className="p-5 border-b border-border-subtle flex items-center justify-between">
 <h3 className="text-base font-semibold text-text-primary">Recent Reviews</h3>
 <Link to="/admin/reviews" className="text-xs text-primary hover:text-text-primary flex items-center transition-colors">
 View All <ChevronRight className="h-3 w-3 ml-1" />
 </Link>
 </div>
 ...
 return (
 <div key={review.review_id} className="bg-bg-card border border-border-subtle rounded-xl p-4 flex flex-col gap-2">
 <div className="flex justify-between items-start">
 <div>
 <p className="text-sm font-medium text-text-primary">{(review as any).user_name || review.user_id}</p>
 <p className="text-xs text-text-secondary truncate max-w-[200px]">on {product?.name || review.product_id}</p>
 </div>
 <div className="flex text-yellow-400">
 {[...Array(5)].map((_, i) => (
 <Star key={i} className={`h-3 w-3 ${i < review.rating ? 'fill-current': 'text-text-secondary'}`} />
 ))}
 </div>
 </div>
 ...
 <Link to="/admin/products" className="text-xs text-primary hover:text-text-primary flex items-center transition-colors">
 Manage <ChevronRight className="h-3 w-3 ml-1" />
 </Link>
 </div>
 <div className="overflow-x-auto">
 ...
 <span className="truncate max-w-[150px] font-medium text-text-primary">{product.name}</span>
 </td>
 <td className="px-5 py-3">{product.category || '-'}</td>
 <td className="px-5 py-3 text-right text-text-primary">{formatCurrency(product.price)}</td>
 </tr>
 ))}
 </tbody>
 </table>
