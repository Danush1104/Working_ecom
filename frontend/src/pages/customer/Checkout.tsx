            ...
            <div className="bg-white dark:bg-bg-card p-6 md:p-8 rounded-[32px] shadow-soft border border-gray-100 dark:border-border-subtle">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent transition-colors"
                ...
                {PAYMENT_METHODS.map((method) => (
                  ...
                  >
                    <input 
                      ...
                      className="sr-only"
                    />
                    <method.icon className={`h-6 w-6 mr-4 ${paymentMethod === method.id ? 'text-primary' : 'text-gray-400'}`} />
                    <span className="font-medium text-gray-900 dark:text-white">{method.label}</span>
                    <div className={`ml-auto w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === method.id ? 'border-primary' : 'border-gray-300 dark:border-gray-600'}`}>
          ...
          <div className="space-y-4 mb-6 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
            {enrichedCart.map((item) => (
              <div key={item.product_id} className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-xl bg-gray-50 dark:bg-gray-900 overflow-hidden shrink-0">
                  <img 
