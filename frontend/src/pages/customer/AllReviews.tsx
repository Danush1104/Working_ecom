...
export default function AllReviews() {
  ...
  return (
    ...
    >
      <div className="pt-2">
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-cyan-400 transition-colors focus:outline-none rounded-lg px-2 py-1 -ml-2">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <div className="flex text-yellow-400">
                          {[1, 2, 3, 4, 5].map(star => (
                            <Star key={star} className={`h-3.5 w-3.5 ${star <= review.rating ? 'fill-current' : 'text-gray-700'}`} />
                          ))}
                        </div>
                      </div>
                    </div>
