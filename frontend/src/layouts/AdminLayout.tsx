...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary flex selection:bg-primary/30 font-sans">
      <ScrollToTop />
      ...
      >
        <button 
          ...
          className="absolute -right-3 top-20 bg-bg-card border border-border-subtle rounded-full p-1 text-text-secondary hover:text-white hover:border-primary/50 transition-colors z-30 shadow-soft"
        >
        ...
        <div className="h-20 flex items-center justify-center px-4 border-b border-border-subtle">
          <Link to="/admin" className="flex items-center gap-3 group focus:outline-none">
            ...
            {!isSidebarCollapsed && (
              <span className="text-xl font-space font-bold tracking-tight text-white group-hover:text-primary transition-colors">
                STOREFRONT
              </span>
            )}
          </Link>
          ...
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                <item.icon className={`h-5 w-5 flex-shrink-0 transition-colors ${isActive ? 'text-primary drop-shadow-[0_0_8px_rgba(21,216,255,0.5)]' : 'text-text-secondary group-hover:text-white'}`} />
                {!isSidebarCollapsed && (
                  <span>{item.name}</span>
                )}
                {/* Tooltip for collapsed state */}
                {isSidebarCollapsed && (
                  <div className="absolute left-14 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity bg-bg-secondary text-white text-xs py-1 px-2 rounded border border-border-subtle whitespace-nowrap z-50">
                    {item.name}
                  </div>
                )}
              </Link>
        ...
        <div className="p-4 border-t border-border-subtle">
          <div className={`flex items-center gap-3 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            <div className="h-10 w-10 rounded-full bg-bg-secondary border border-border-subtle flex items-center justify-center font-bold text-white shadow-soft flex-shrink-0">
              {initial}
            </div>
            {!isSidebarCollapsed && (
        ...
        <header className="h-20 flex items-center justify-between px-4 sm:px-8 bg-bg-primary/80 backdrop-blur-xl border-b border-border-subtle sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button 
              ...
              className="md:hidden p-2 -ml-2 text-text-secondary hover:text-white focus:outline-none rounded-lg transition-colors"
            >
              <Menu className="h-6 w-6" />
            </button>
            <div className="hidden sm:block">
              <h1 className="text-xl font-space font-semibold text-white tracking-tight">{currentPage}</h1>
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <button className="p-2 text-text-secondary hover:text-white hover:bg-white/5 rounded-full transition-colors focus:outline-none">
            ...
            <div className="relative" ref={profileRef}>
              <button 
                ...
                className="flex items-center gap-2 p-1 rounded-full hover:bg-white/5 focus:outline-none transition-colors"
              >
                <div className="h-8 w-8 rounded-full bg-bg-secondary border border-border-subtle flex items-center justify-center font-bold text-sm text-white hover:border-primary/50 transition-colors">
                  {initial}
                </div>
              </button>
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    transition={{ duration: 0.15 }}
                    className="absolute right-0 mt-2 w-48 bg-bg-card/95 backdrop-blur-xl rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.5)] border border-border-subtle overflow-hidden z-50"
                  >
                    <div className="p-2">
                      ...
                      <Link to="/admin" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-2 w-full p-2.5 text-sm text-text-secondary hover:text-white hover:bg-white/5 rounded-xl transition-colors">
                    ...
                    <div className="p-2 border-t border-border-subtle">
                      <button onClick={() => { setIsProfileOpen(false); logout(); }} className="flex items-center gap-2 w-full p-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-colors">
          ...
          >
            ...
            >
              <div className="flex h-20 items-center justify-between px-6 border-b border-border-subtle">
                ...
                <button 
                  onClick={() => setIsSidebarOpen(false)}
                  className="p-2 -mr-2 text-text-secondary hover:text-white rounded-lg transition-colors"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
