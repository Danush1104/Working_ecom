              ...
              data.map((item, idx) => (
                <motion.tr 
                  ...
                  className={`group transition-all ${
                    onRowClick 
                      ? 'cursor-pointer hover:bg-white/5 hover:shadow-[inset_2px_0_0_0_#15d8ff]' 
                      : 'hover:bg-white/5'
                  }`}
                >
                  {columns.map((col, colIdx) => (
                    <td key={colIdx} className={`px-5 py-4 text-sm text-text-primary whitespace-nowrap ${col.className || ''}`}>
