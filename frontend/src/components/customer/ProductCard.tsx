...
export function ProductCard({ 
 ...
 const renderStars = () => {
 if (total_reviews === 0) return <span className="text-text-secondary font-normal">☆☆☆☆☆ No reviews</span>;
 ...
 <span className="font-semibold text-text-primary">{average_rating.toFixed(1)}</span>
 <span className="text-text-secondary ml-1">({total_reviews})</span>
 </>
 );
 };
 ...
 <motion.div
 whileHover={{ y: -4 }}
 className="group relative bg-bg-card/60 backdrop-blur-xl rounded-[24px] border border-border-subtle hover:border-cyan-500/30 overflow-hidden flex flex-col transition-all duration-500 hover:shadow-[0_8px_32px_rgba(21,216,255,0.1)]"
 >
 <Link to={`/product/${id}`} className="block relative aspect-[4/5] overflow-hidden bg-bg-card/5">
 <img
 ...
 : 'bg-bg-card/10 border-white/20 hover:bg-bg-card/20 hover:border-white/30'
 } border`}
 >
 <motion.div
 ...
 <div className="px-6 py-2.5 rounded-full bg-bg-card/10 border border-white/20 text-white font-medium flex items-center gap-2 backdrop-blur-md translate-y-4 group-hover:translate-y-0 transition-all duration-300">
 ...
 <h3 className="text-lg font-semibold text-text-primary line-clamp-2 leading-tight mb-1">
 {name}
 </h3>
 </Link>
 ...
 ${!is_active 
 ? 'bg-bg-card/5 border border-border-subtle text-text-secondary cursor-not-allowed'
