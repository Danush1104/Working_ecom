...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if "Records" in event:
        # SQS event processing (Product service does not use this, but we log and handle)
        update_log_context(
            lambda_request_id=getattr(context, "aws_request_id", None)
        )
        logger.warning("Product Service received SQS event, which is not expected for this service.")
        return {
            "batchItemFailures": []
        }
